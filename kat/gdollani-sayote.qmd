...
for(i in 1:25)
{
  fit_sayote <- auto.arima(sayote, xreg=fourier(sayote, K=i), seasonal=FALSE)
...
```{r}
fc_arima_sayote <- forecast(bestfit_sayote, xreg=fourier(sayote, K=2, h=26))
...
```{r}
accuracy(fc_sayote_ts)
```

```{r}
accuracy(fc_arima_sayote_ts)
```
